...
public class GoogleVideoSitemapUrl extends WebSitemapUrl {
    ...
    public static class Options extends AbstractSitemapUrlOptions<GoogleVideoSitemapUrl, Options> {
        ...
        public Options title(String title) {
            if (title != null && title.length() > 100) {
                throw new SitemapGeneratorException("Video title is limited to 100 characters: " + title);
            }
        ...
        public Options description(String description) {
            if (description != null && description.length() > 2048) {
                throw new SitemapGeneratorException("Truncate video descriptions to 2048 characters: " + description);
            }
        ...
        public Options rating(Double rating) {
            if (rating != null && (rating < 0 || rating > 5.0)) {
                throw new SitemapGeneratorException("Rating must be between 0.0 and 5.0:" + rating);
            }
        ...
        /**
         * The date the video was first published, in {@link W3CDateTimeFormatter}.
         */
        public Options publicationDate(Temporal publicationDate) {
        ...
        public Options category(String category) {
            if (category != null && category.length() > 256) {
                throw new SitemapGeneratorException("Video category is limited to 256 characters: " + title);
            }
        ...
        public Options durationInSeconds(int durationInSeconds) {
            if (durationInSeconds < 0 || durationInSeconds > 28800) {
                throw new SitemapGeneratorException("Duration must be between 0 and 28800 (8 hours):" + durationInSeconds);
            }
    ...
    public GoogleVideoSitemapUrl(Options options) {
        ...
        if (playerUrl == null && contentUrl == null) {
            throw new SitemapGeneratorException("You must specify either contentUrl or playerUrl or both; neither were specified");
        }
        allowEmbed = convertBooleanToYesOrNo(options.allowEmbed);
        if (playerUrl != null && allowEmbed == null) {
            throw new SitemapGeneratorException("allowEmbed must be specified if playerUrl is specified");
        }
        ...
        if (tags != null && tags.size() > 32) {
            throw new SitemapGeneratorException("A maximum of 32 tags is permitted");
        }
